...
const WhatsAppIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" className="h-5 w-5 fill-[#25D366]">
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const handleShare = async (platform: 'whatsapp' | 'facebook' | 'instagram' | 'native', e: React.MouseEvent) => {
    ...
    const shareText = `Check out this beautiful ${product.name} at SareeDukan!`;
          ...
          <div className="flex gap-2">
             <Button 
              ...
              <Popover>
                ...
                <PopoverContent 
                  className="w-auto p-3 rounded-[2rem] shadow-2xl border-primary/10 backdrop-blur-xl bg-white/95" 
                  align="start" 
                  side="bottom"
                  sideOffset={10}
                >
                  <div className="flex flex-row gap-3">
                    ...
                    <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full hover:bg-pink-50" onClick={(e) => handleShare('instagram', e)}>
                      <Instagram className="h-5 w-5 text-pink-600" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full hover:bg-primary/5" onClick={(e) => handleShare('native', e)}>
